...
export function getSubmissionAttribution(): SubmissionAttribution {
  ...
  return {
    ...firstTouch,
    submissionPage: typeof window === "undefined"
      ? "Not provided"
...
export function appendSubmissionAttribution(formData: FormData) {
  const attribution = getSubmissionAttribution();
  const fields: Record<string, string | undefined> = {
    source_page: attribution.submissionPage,
    initial_landing_page: attribution.initialLandingPage,
    initial_referrer: attribution.initialReferrer,
    submission_page: attribution.submissionPage,
    submitted_at: attribution.submittedAt,
  ...
  for (const [name, value] of Object.entries(fields)) {
    formData.set(name, value ?? "Not provided");
  }
  return formData;
}
